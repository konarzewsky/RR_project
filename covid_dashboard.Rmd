...
if (dim(data_time_tests)[1] != 0) {
  ggplot(data = head(data_time_tests,-1), aes(x = date, y = tests)) + 
...
```{r plot_tests2}
data_ak$date_char = as.character(data_ak$date)
data1 <- data_ak[data_ak$administrative_area_level_1 %in% country,]
data1[is.na(data1$tests), ]$tests <- 0
data1 <- data1[,c("date", "date_char", "tests", "administrative_area_level_1")]
#sum(is.na(data1$tests)) #No missings
#sum(is.na(data1$administrative_area_level_1))
...
colnames(data1_wide)
data1_wide[, c(3:29)] <- 
  lapply(data1_wide[, c(3:29)], as.numeric)

data1_wide$tests_all <- data1_wide[, c(3:29)] %>% rowSums()
data1_wide$date_month <- cut(as.Date(data1_wide$date_char), "month")
data1_wide$date_month_char = as.character(data1_wide$date_month)

data_time_tests2 <- sqldf(
  "select
    date_month_char as date,
...
ggplot(data = data_time_tests2, aes(x = date, y = tests)) + 
  geom_bar(position="stack", stat="identity") +
  labs(
    x = "",
    y = "",
