...
```{r, plots_deaths_vaccines, fig.show="hold", out.width="50%", fig.align = 'center'}
data <- data %>%
  mutate_at(vars(deaths), list(~ .x - lag(.x)))
...
if (dim(data_time_deaths)[1] != 0) {
  ggplot(data = head(data_time_deaths,-1), aes(x = date, y = deaths)) + 
    geom_bar(position="stack", stat="identity") +
    labs(
      x = "",
      y = "",
      title = "monthly number of deaths (thousands)",
